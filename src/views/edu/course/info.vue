    ...
    <el-form label-width="120px">
      ...
      <el-form-item label="课程封面">
        <el-upload
          ...
          <img :src="courseInfo.cover" height="200px">
        </el-upload>
  ...
  data() {
    return {
      ...
      initInfo: {
        ...
        lessonNum: 0,
        description: '',
        cover: 'https://edu-online-demo1.oss-cn-beijing.aliyuncs.com/default.jpg',
        price: 0
      },
      courseInfo: {},
      teacherList: [], // 封装所有讲师
  ...
  methods: {
    ...
    getInfo() {
      course.getCourseInfo(this.courseId)
        .then(response => {
          // 再courseInfo课程信息里面,包含一级分类id 二级分类id
          ...
          subject.getSubjectList()
            .then(response => {
              ...
              for (var i = 0; i < this.subjectOneList.length; i++) {
                // 获取每个一级分类
                var oneSubject = this.subjectOneList[i]
                // 比较当前courseInfo里面一级分了id和所有的一级分类id
                if (oneSubject.id === this.courseInfo.subjectParentId) {
                  // 如果相同,获取一级分类所有的二级分类(选中内容会根据id自动选中)
                  this.subjectTwoList = oneSubject.children
                }
              }
            })
          // 初始化所有讲师(因为有id 初始化后会自动回显)
          this.getListTeacher()
        })
    }
  }
